...
CREATE TABLE IF NOT EXISTS `notasinnsolDB`.`Companies` (
  ...
  PRIMARY KEY (`CO_Id`),
  UNIQUE INDEX `CO_Email_UNIQUE` (`CO_Email` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `notasinnsolDB`.`Company_Theme` (
  ...
  UNIQUE INDEX `CO_Id_UNIQUE` (`CT_CO_Id` ASC) VISIBLE,
  INDEX `FK_CO_TH_Theme_Id_idx` (`CT_TH_Id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `notasinnsolDB`.`User_Packages` (
  ...
  UNIQUE INDEX `UP_US_Id_UNIQUE` (`UP_US_Id` ASC) VISIBLE,
  INDEX `FK_UP_Package_Id_idx` (`UP_PC_Id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `notasinnsolDB`.`Notes` (
  ...
  `NO_CL_Name` VARCHAR(150) NOT NULL,
  `NO_CL_Email` VARCHAR(255) NOT NULL,
  `NO_CL_Direction` VARCHAR(512) NOT NULL,
  ...
  `NO_Riva` DOUBLE NOT NULL,
  `NO_Isr` DOUBLE NOT NULL,
...
CREATE TABLE IF NOT EXISTS `notasinnsolDB`.`Company_Users` (
  ...
  INDEX `FK_CU_Company_Id_idx` (`CU_CO_Id` ASC) VISIBLE,
  INDEX `FK_CU_User_Id_idx` (`CU_US_Id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `notasinnsolDB`.`User_Notes` (
  ...
  INDEX `FK_UN_User_Id_idx` (`UN_US_Id` ASC) VISIBLE,
  INDEX `FK_UN_Note_Id_idx` (`UN_NO_Id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `notasinnsolDB`.`Note_Type` (
  ...
  UNIQUE INDEX `NT_NO_Id_UNIQUE` (`NT_NO_Id` ASC) VISIBLE,
  INDEX `FK_NT_Type_Id_idx` (`NT_TY_Id` ASC) VISIBLE,
  ...
  CONSTRAINT `FK_NT_Type_Id`
    ...
    REFERENCES `notasinnsolDB`.`TypesOfNotes` (`TY_Id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `notasinnsolDB`.`Note_Products` (
  ...
  INDEX `FK_NP_Note_Id_idx` (`NP_NO_Id` ASC) VISIBLE,
  INDEX `FK_NP_Product_Id_idx` (`NP_PR_Id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `notasinnsolDB`.`User_Type` (
  ...
  UNIQUE INDEX `UT_US_Id_UNIQUE` (`UT_US_Id` ASC) VISIBLE,
  INDEX `FK_UT_Type_User_Id_idx` (`UT_TY_Id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `notasinnsolDB`.`Company_Products` (
  ...
  INDEX `FK_CP_Company_Id_idx` (`CP_CO_Id` ASC) VISIBLE,
  INDEX `FK_CP_Product_Id_idx` (`CP_PR_Id` ASC) VISIBLE,
...
-- -----------------------------------------------------
-- Table `notasinnsolDB`.`Prospectuses`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `notasinnsolDB`.`Prospectuses` (
  `PP_Id` INT NOT NULL,
  `PP_Name` VARCHAR(150) NOT NULL,
  `PP_Number` DOUBLE NOT NULL,
  `PP_Direction` VARCHAR(512) NOT NULL,
  `PP_State` TINYINT(1) NOT NULL,
  `PP_Email` VARCHAR(150) NOT NULL,
  PRIMARY KEY (`PP_Id`),
  UNIQUE INDEX `PP_Email_UNIQUE` (`PP_Email` ASC) VISIBLE)
ENGINE = InnoDB;
